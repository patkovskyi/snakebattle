    ...
    do {
      ...
      for (Direction d : Direction.onlyDirections()) {
        ...
        if (nondir[newP.getX()][newP.getY()] == nondir[p.getX()][p.getY()] - 1) {
          ...
          if (bestP == null
              || lucrativity[newP.getX()][newP.getY()] > lucrativity[bestP.getX()][bestP.getY()]) {
            bestP = newP;
          }
        }
      }
  ...
  public int[][] getDistances() {
    ...
    PriorityQueue<Point> openSet =
        new PriorityQueue<>(
            (p1, p2) ->
                Integer.compare(
                    lucrativity[p2.getX()][p2.getY()], lucrativity[p1.getX()][p1.getY()]));
    openSet.add(head);
          ...
          for (Direction ndir : Direction.onlyDirections()) {
            if (step > 0 || ndir != headDirection.inverted()) {
              ...
              if (!isNotPassableOrRisky(np) && step + 1 < dist[np.getX()][np.getY()]) {
                lucrativity[np.getX()][np.getY()] =
                    Math.max(
                        lucrativity[p.getX()][p.getY()] + getPointLucrativity(np),
                        lucrativity[np.getX()][np.getY()]);
                openSet.add(np);
              }
            }
